@extends('layouts.app')
@section('title', isset($aktivitas) ? 'Edit Aktivitas Staf' : 'Tambah Aktivitas Staf')
@section('page-title', isset($aktivitas) ? 'Edit Aktivitas Staf Administrasi' : 'Tambah Aktivitas Staf Administrasi')
@section('content')
    ...
    <div class="col-lg-8">
        <div class="card-clean card">
            ...
            <div class="card-body p-4">
                <form method="POST" action="{{ isset($aktivitas) ? route('admin.aktivitas-staf-administrasi.update', $aktivitas) : route('admin.aktivitas-staf-administrasi.store') }}" enctype="multipart/form-data">
                    @csrf
                    @if(isset($aktivitas)) @method('PUT') @endif

                    <div class="row g-3">
                        <div class="col-md-12">
                            <label class="form-label fw-semibold" style="font-size:.85rem">Staf Administrasi <span class="text-danger">*</span></label>
                            <select name="staf_administrasi_id" class="form-select @error('staf_administrasi_id') is-invalid @enderror" required>
                                <option value="">-- Pilih Staf Administrasi --</option>
                                @foreach($daftarStaf as $staf)
                                    <option value="{{ $staf->id }}" {{ old('staf_administrasi_id', $aktivitas->staf_administrasi_id ?? '') == $staf->id ? 'selected' : '' }}>{{ $staf->nama_lengkap }}</option>
                                @endforeach
                            </select>
                            @error('staf_administrasi_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="kategori" class="form-select @error('kategori') is-invalid @enderror" required>
                                ...
                                @foreach(\App\Models\AktivitasStafAdministrasi::kategoriGrouped() as $group => $items)
                    ...
                    <div class="d-flex gap-2 mt-4">
                        ...
                        <a href="{{ route('admin.aktivitas-staf-administrasi.index') }}" class="btn btn-outline-secondary">Batal</a>
                    </div>
                </form>
            </div>
        </div>
